    ...
    public static function completion($arrayCompletion)
    {
        try {
            ...
            $headers = [
                ...
                'Authorization: Bearer '.$GLOBALS['DEEPINFRA']
            ];

            $data['model'] =  "meta-llama/Meta-Llama-3.1-8B-Instruct";
            $data['messages'] = $arrayCompletion;
            $data['max_tokens'] = 200;
            $data['min_new_tokens'] = 200;
            $data['temperature'] = 0.00001;
            $data['do_sample'] = true;
            $data['stream'] = false;
        ...
        try {
            ...
            if ($err) {
                ...
                if(isset($saida['choices']) && isset($saida['choices'][0]['message']['content']) && !empty($saida['choices'][0]['message']['content']))return $saida['choices'][0]['message']['content'];
                    ...
                    throw new Exception("Saida LLM Vazia DeepInfra");
                }
            }
        } catch (\Exception $ex) {
