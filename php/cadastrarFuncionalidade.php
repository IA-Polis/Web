...
if(isset($_SESSION['PERMISSOES']['cadastrarFuncionalidade'])){
	if($_POST['btnCadastrarFuncionalidade'])
	{
		$newFuncionalidade = new Classe\Funcionalidade();
		$newFuncionalidade->setNome($_POST['txtNome']);
		$newFuncionalidade->setDescricao($_POST['txtDescricao']);
		if ($_POST['txtNome'])
		{
			try
			{
				$saida =  "<div class='alert alert-success alert-dismissable'>".$newFuncionalidade->incluir()."</div>";
